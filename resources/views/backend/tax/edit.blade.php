                ...
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3">
                        <div class="d-flex justify-content-between">
                            <h6 class="text-white text-capitalize ps-3">Edit Tax</h6>
                        </div>
                    </div>
                </div>

                <div class="card-body px-4 pb-2">
                    <form action="{{ route('panel.tax.update', $tax->id) }}" method="post" class="p-4">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control border px-3 @error('name') is-invalid @enderror"
                                        name="name" id="name" value="{{ old('name', $tax->name) }}" required>
                                    @error('name')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="number" step="0.01" class="form-control border px-3 @error('rate') is-invalid @enderror"
                                        name="rate" id="rate" value="{{ old('rate', $tax->rate) }}" required>
                                    @error('rate')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
